...
// 0 01 111 0 0 -> temp comp disabled, low performance mode, 10 ODR, fast ODR disabled, self-test disabled
static const uint8_t SET_MAGNETO_ODR_DATA = 0x3C;

static const uint8_t ACTIVATE_MAGNETO_CMD = M3_CTRL_REGISTER_ADDR | WRITE_CMD_MASK;
// 1 0 0 00 1 01 -> I2C Disable, Low power mode disabled, SPI write enable, Continuous-conversion mode
...
static const uint8_t READ_MAGNETO_X_HIGH_CMD = 0x29 | READ_CMD_MASK | MAGNETO_MASK;
static const uint8_t READ_MAGNETO_Y_LOW_CMD = 0x2A | READ_CMD_MASK | MAGNETO_MASK;
static const uint8_t READ_MAGNETO_Y_HIGH_CMD = 0x2B | READ_CMD_MASK | MAGNETO_MASK;
static const uint8_t READ_MAGNETO_Z_LOW_CMD = 0x2C | READ_CMD_MASK | MAGNETO_MASK;
static const uint8_t READ_MAGNETO_Z_HIGH_CMD = 0x2D | READ_CMD_MASK | MAGNETO_MASK;
...
void readAccelGyroMagnetismTask(void const* arg)
{
    ...
    for (;;)
    {
        ...
        HAL_GPIO_WritePin(MAG_CS_GPIO_Port, MAG_CS_Pin, GPIO_PIN_RESET);
        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_X_LOW_CMD, &dataBuffer[0], 1, CMD_TIMEOUT);


        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_X_HIGH_CMD, &dataBuffer[1], 1, CMD_TIMEOUT);


        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_Y_LOW_CMD, &dataBuffer[2], 1, CMD_TIMEOUT);


        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_Y_HIGH_CMD, &dataBuffer[3], 1, CMD_TIMEOUT);


        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_Z_LOW_CMD, &dataBuffer[4], 1, CMD_TIMEOUT);

        HAL_SPI_TransmitReceive(&hspi1, &READ_MAGNETO_Z_HIGH_CMD, &dataBuffer[5], 1, CMD_TIMEOUT);
        HAL_GPIO_WritePin(MAG_CS_GPIO_Port, MAG_CS_Pin, GPIO_PIN_SET);
